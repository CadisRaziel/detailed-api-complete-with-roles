package com.vitu.full_api.config;
...
//http://localhost:8081/security-api/swagger-ui/index.html
...
public class SwaggerConfigurations { // Configurações do SWAGGER !
    ...
    public OpenAPI almadaAPI() { // Nesse método estão algumas configurações que apareceram na interface do swagger e confs de autorização.
        ...
        return new OpenAPI()
                ...
                .components(new Components().addSecuritySchemes("bearer-key", new SecurityScheme().type(SecurityScheme.Type.HTTP) //-> para poder testar direto no swagger (vai habilitar o campo para por o bearer)
                        .scheme("bearer")
                        .bearerFormat("JWT")))
